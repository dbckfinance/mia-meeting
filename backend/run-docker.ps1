...
$AppProjectName = "reikn-backend"
$AppContainerName = "reikn-backend"
$DefaultPort = 8178
$DefaultAppPort = 5167
...
function Invoke-StartCommand {
    ...
    $composeArgs += @("whisper-server", "reikn-backend")
    ...
    if ($DryRun) {
        ...
        if ($detach) {
            ...
            & docker-compose -f $ComposeFile --profile default up -d whisper-server reikn-backend
            ...
            & docker-compose -f $ComposeFile --profile default up whisper-server reikn-backend
...
function Invoke-LogsCommand {
    ...
    $serviceMap = @{
        ...
        "app" = "reikn-backend"
        "backend" = "reikn-backend"
        "meeting" = "reikn-backend"
    }
    ...
    if ($service) {
        ...
        $validServices = @("whisper-server", "reikn-backend")
        if ($service -notin $validServices) {
            Write-Warn "Unknown service: $service"
            Write-Info "Available services: whisper-server, reikn-backend"
        ...
        $logArgs += @("whisper-server", "reikn-backend")
    }
