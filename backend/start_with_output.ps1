...
Write-Host "====================================="
Write-Host "Reikn Backend Startup"
Write-Host "====================================="
...
Write-Host "====================================="
Write-Host "Starting Reikn Backend"
Write-Host "====================================="
...
# Check if reikn-frontend is installed
$frontendInstalled = $false
$frontendPath = $null

# Check common installation paths for reikn-frontend
$possiblePaths = @(
    "$env:LOCALAPPDATA\Programs\reikn-frontend\reikn-frontend.exe",
    "$env:LOCALAPPDATA\Programs\reikn\reikn-frontend.exe",
    "$env:ProgramFiles\reikn-frontend\reikn-frontend.exe",
    "${env:ProgramFiles(x86)}\reikn-frontend\reikn-frontend.exe",
    "$env:APPDATA\reikn-frontend\reikn-frontend.exe"
)
...
try {
    ...
    if ($regPath) {
        ...
        if (-not $frontendPath -and $regPath.InstallLocation) {
            ...
            $possibleExeNames = @("reikn-frontend.exe", "reikn.exe")
...
if ($frontendInstalled) {
    Write-Host "Reikn frontend application is installed."
    if ($frontendPath) {
        Write-Host "Location: $frontendPath"
        
        # Ask if user wants to launch the frontend
        $launchFrontend = Read-Host "Do you want to launch the Reikn frontend application? (Y/N)"
        if ($launchFrontend -eq 'Y' -or $launchFrontend -eq 'y') {
            Write-Host "Launching Reikn frontend..."
            Start-Process -FilePath $frontendPath
            Write-Host "Reikn frontend launched successfully."
        }
    }
} else {
    Write-Host "Reikn frontend application is not installed."
    Write-Host ""
    $installFrontend = Read-Host "Would you like to download and install the Reikn frontend application? (Y/N)"
        ...
        try {
            ...
            if ($setupAsset) {
                ...
                if ($installerProcess.ExitCode -eq 0) {
                    ...
                    # Check if reikn is now installed and launch it
