...
function Find-ExistingDatabases {
    ...
    $commonPaths = @(
        "$env:USERPROFILE\.reikn\meeting_minutes.db",
        "$env:USERPROFILE\Documents\reikn\meeting_minutes.db",
    ...
    if ($env:HOMEBREW_PREFIX) {
        $commonPaths += "$env:HOMEBREW_PREFIX/Cellar/reikn-backend/*/backend/meeting_minutes.db"
    }
